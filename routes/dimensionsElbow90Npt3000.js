...
async function dimensionsElbow90Npt3000(agent) {
    ...
    if (_.isUndefined(context)) {
        ...
        if (!sizeOne) {
            ...
            await Elb90npt.findOne({sizeOne, class: 3000}, function (err, res) {
                if (!!err || !res) {
                    return dontKnowResponce(agent);
                } else {
                    return dimensionsResponce(agent, unit, res.dimensions, `${sizeOne} ${translate('elb90npt', locale)} class 3000`, 'elb90npt.png')
                }
            });
        }
    }
...
module.exports = dimensionsElbow90Npt3000;
