...
async function dimensionsCrossSw3000(agent) {
    ...
    if (_.isUndefined(context)) {
        ...
        if (!sizeOne) {
            ...
            await CrossSw.findOne({sizeOne, class: 3000}, function (err, res) {
                if (!!err || !res) {
                    return dontKnowResponce(agent);
                } else {
                    return dimensionsResponce(agent, unit, res.dimensions, `${sizeOne} ${translate('crosssw', locale)} class 3000`, 'crosssw.png')
                }
            });
        }
    }
...
module.exports = dimensionsCrossSw3000;