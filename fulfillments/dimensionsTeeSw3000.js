...
async function dimensionsTeeSw3000(agent) {
    ...
    if (_.isUndefined(context)) {
        ...
        if (!sizeOne) {
            ...
            await TeeSw.findOne({sizeOne, class: 3000}, function (err, res) {
                if (!!err || !res) {
                    return dontKnowResponce(agent);
                } else {
                    return dimensionsResponce(agent, unit, res.dimensions, `${sizeOne} ${translate('teesw', locale)} class 3000`, 'teesw.png')
                }
            });
        }
    }
...
module.exports = dimensionsTeeSw3000;