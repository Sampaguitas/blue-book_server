...
async function dimensionsCrossNpt6000(agent) {
    ...
    if (_.isUndefined(context)) {
        ...
        if (!sizeOne) {
            ...
            await CrossNpt.findOne({sizeOne, class: 6000}, function (err, res) {
                if (!!err || !res) {
                    return dontKnowResponce(agent);
                } else {
                    return dimensionsResponce(agent, unit, res.dimensions, `${sizeOne} ${translate('crossnpt', locale)} class 6000`, 'crossnpt.png')
                }
            });
        }
    }
...
module.exports = dimensionsCrossNpt6000;