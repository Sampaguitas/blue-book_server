...
async function dimensionsElbow90Sw9000(agent) {
    ...
    if (_.isUndefined(context)) {
        ...
        if (!sizeOne) {
            ...
            await Elb90Sw.findOne({sizeOne, class: 9000}, function (err, res) {
                if (!!err || !res) {
                    return dontKnowResponce(agent);
                } else {
                    return dimensionsResponce(agent, unit, res.dimensions, `${sizeOne} ${translate('elb90sw', locale)} class 9000`, 'elb90sw.png')
                }
            });
        }
    }
...
module.exports = dimensionsElbow90Sw9000;
