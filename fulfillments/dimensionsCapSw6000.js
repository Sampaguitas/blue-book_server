...
async function dimensionsCapSw6000(agent) {
    ...
    if (_.isUndefined(context)) {
        ...
        if (!sizeOne) {
            ...
            await CapSw.findOne({sizeOne, class: 6000}, function (err, res) {
                if (!!err || !res) {
                    return dontKnowResponce(agent);
                } else {
                    return dimensionsResponce(agent, unit, res.dimensions, `${sizeOne} ${translate('capsw', locale)} class 6000`, 'capsw.png')
                }
            });
        }
    }
...
module.exports = dimensionsCapSw6000;
