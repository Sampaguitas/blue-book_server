...
async function dimensionsTeeSw9000(agent) {
    ...
    if (_.isUndefined(context)) {
        ...
        if (!sizeOne) {
            ...
            await TeeSw.findOne({sizeOne, class: 9000}, function (err, res) {
                if (!!err || !res) {
                    return dontKnowResponce(agent);
                } else {
                    return dimensionsResponce(agent, unit, res.dimensions, `${sizeOne} ${translate('teesw', locale)} class 9000`, 'teesw.png')
                }
            });
        }
    }
...
module.exports = dimensionsTeeSw9000;